...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<Object>& detections, const vector<IMU::Point>& vImuMeas, string filename){
    return TrackRGBD(im, depthmap, timestamp, vImuMeas, filename);
}
