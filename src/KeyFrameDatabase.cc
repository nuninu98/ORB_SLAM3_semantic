...
vector<pair<float, size_t>> KeyFrameDatabase::vDetectNBestCandidates(KeyFrame *pKF, vector<KeyFrame*> &vpLoopCand, vector<KeyFrame*> &vpMergeCand, int nNumCandidates){
    vector<pair<float, size_t>> output;
    ...
    for(list<pair<float,KeyFrame*> >::iterator it=lScoreAndMatch.begin(), itend=lScoreAndMatch.end(); it!=itend; it++)
    {
        ...
        lAccScoreAndMatch.push_back(make_pair(accScore,pBestKF));
        output.push_back(make_pair(accScore, pBestKF->mnId));
        if(accScore>bestAccScore)
            bestAccScore=accScore;
    }
